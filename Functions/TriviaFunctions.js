...
module.exports = {
  ...
  parseQuestionToEmbed: (qObject, message) => {
    ...
    let allAnswers = qObject.incorrect_answers; allAnswers.push(cAnswer); shuffle(allAnswers);
    ...
    const cAnswerLetter = b64toText(choices.find(x => x.content === cAnswer).id);

    return [cAnswerLetter, `${cAnswerLetter.toUpperCase()}. ${b64toText(cAnswer)}`]
  },
