...
module.exports = (message, { vc = true, sameVC = true, playing = true, queue = false, songNumber = false } = {}) => {
	const checks = {
		vc: !message.member.voice.channel && vc,
		sameVC: (message.member.voice.channel !== message.guild.me.voice.channel) && sameVC,
		playing: !message.guild.musicData.songDispatcher && playing,
		queue: (message.guild.musicData.queue.length < 1) && queue,
		songNumber:
			(songNumber < 1 || songNumber > (message.guild.musicData.queue.length ? message.guild.musicData.queue.length : 1))  &&
			songNumber !== false,
	};
	const embedData = {
		authorBool: true,
